...
export default function App() {
   ...
   return (
      <div className="min-h-screen bg-[#F7FBFF] text-[#0F172A] relative selection:bg-[#BAE6FD] selection:text-[#0284C7] overflow-x-hidden font-sans">
         ...
         {/* Global Navbar */}
         <Navbar activeSection={activeSection} onNavClick={handleNavClick} onContactClick={() => setContactOpen(true)} />

         {/* Main Flow */}
         <main className="relative z-10">
            {/* 1. Hero Viewport (Asymmetric with 3D Glass Sphere Workspace) */}
            <HeroContainer onExploreWork={() => handleNavClick('experience')} onViewProjects={() => handleNavClick('projects')} onContactClick={() => setContactOpen(true)} />

            {/* 2. Selected Work */}
            <SelectedWorkPreview onSelectProject={(project) => setSelectedProject(project)} />
         ...
         {/* Footer */}
         <Footer activeSection={activeSection} onNavClick={handleNavClick} onContactClick={() => setContactOpen(true)} />
